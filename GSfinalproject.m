...
for j = 1:n50 %makes the temperature at the boundaries equal to zero
    for i = 1:n50
      ...
      if t <= n50 || rem(t,n50) == 0 || rem(t,n50) == 1 || t > (n50)*((n50)-1)
        ...
        K50 =  k50value(i,j,n50); %Code for Gauss Seidel
        k50 = K50*(n50)^2;
...
zlabel('Temperature')
axis tight
title(['Temperature Field using Gauss-Seidel for ' num2str(n50) 'x' num2str(n50)])
...
for j = 1:n100 %makes the temperature at the boundaries equal to zero
    for i = 1:n100
      ...
      if t <= n100 || rem(t,n100) == 0 || rem(t,n100) == 1 || t > (n100)*((n100)-1)
        ...
        K100 =  k100value(i,j,n100); %Code for Gauss Seidel
        k100 = K100*(n100)^2;
...
zlabel('Temperature')
axis tight
title(['Temperature Field using Gauss-Seidel for ' num2str(n100) 'x' num2str(n100)])
...
for j = 1:n200 %makes the temperature at the boundaries equal to zero
    for i = 1:n200
      ...
      if t <= n200 || rem(t,n200) == 0 || rem(t,n200) == 1 || t > (n200)*((n200)-1)
        ...
        K200 =  k200value(i,j,n200); %Code for Gauss Seidel
        k200 = K200*(n200)^2;
...
zlabel('Temperature')
axis tight
title(['Temperature Field using Gauss-Seidel for ' num2str(n200) 'x' num2str(n200)])
...
axis tight
grid on
title('Centre-line Temperature using Gauss-Seidel for all cases')
...
grid on
hold off
title('Log of Residual vs. Iterations using Gauss-Seidel for all cases')
...
grid on
hold off
title('Log of Residual vs. CPU Time using Gauss-Seidel for all cases')
