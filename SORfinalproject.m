...
for j = 1:n50 %makes the temperature at the boundaries equal to zero
    for i = 1:n50
      ...
      if t <= n50 || rem(t,n50) == 0 || rem(t,n50) == 1 || t > (n50)*((n50)-1)
        ...
        K50 =  k50value(i,j,n50); %Code for SOR
        k50 = (w50)*K50*(n50)^2;
        T_new50(t) = w50*f50value(i,j,n50);
        T_new50(t) = T_new50(t)+ k50*T_new50(t-1)+ k50*T_new50(t+1) + k50*T_new50(t+n50)+ k50*T_new50(t-n50);
        T_new50(t) = T_new50(t)/(4*(w50)*(K50)*(n50)^2);
        T_new50(t) = (w50)*T_new50(t) + (1-w50)*T_050(t);    
      end
    end  
end
...
zlabel('Temperature')
axis tight
title(['Temperature Field using SOR for ' num2str(n50) 'x' num2str(n50)])
...
n100 = 100;
k100 = n100^2;
w100 = (4/(2+((sqrt(4-((2*cos(pi/n100))^2))))));
...
for j = 1:n100 %makes the temperature at the boundaries equal to zero
    for i = 1:n100
      ...
      if t <= n100 || rem(t,n100) == 0 || rem(t,n100) == 1 || t > (n100)*((n100)-1)
        ...
        K100 =  k100value(i,j,n100); %Code for SOR
        k100 = (w100)*K100*(n100)^2;
        T_new100(t) = (w100)*f100value(i,j,n100);
        T_new100(t) = T_new100(t)+ k100*T_new100(t-1)+ k100*T_new100(t+1) + k100*T_new100(t+n100)+ k100*T_new100(t-n100);
        T_new100(t) = T_new100(t)/(4*(w100)*(K100)*(n100)^2);
        T_new100(t) = (w100)*T_new100(t) + (1-w100)*T_100(t); 
...
zlabel('Temperature')
axis tight
title(['Temperature Field using SOR for ' num2str(n100) 'x' num2str(n100)])
...
n200 = 200;
k200 = n200^2;
w200 = (4/(2+((sqrt(4-((2*cos(pi/n200))^2))))));
...
for j = 1:n200 %makes the temperature at the boundaries equal to zero
    for i = 1:n200
      ...
      if t <= n200 || rem(t,n200) == 0 || rem(t,n200) == 1 || t > (n200)*((n200)-1)
        ...
        K200 =  k200value(i,j,n200); %Code for SOR
        k200 = (w200)*K200*(n200)^2;
        T_new200(t) = (w200)*f200value(i,j,n200);
        T_new200(t) = T_new200(t)+ k200*T_new200(t-1)+ k200*T_new200(t+1) + k200*T_new200(t+n200)+ k200*T_new200(t-n200);
        T_new200(t) = T_new200(t)/(4*(w200)*(K200)*(n200)^2);
        T_new200(t) = (w200)*T_new200(t) + (1-w200)*T_200(t);
...
zlabel('Temperature')
axis tight
title(['Temperature Field using SOR for ' num2str(n200) 'x' num2str(n200)])
...
figure(4)
hold on
plot(p50, X50, 'Linewidth',2)
plot(p100, X100, 'Linewidth',2)
plot(p200, X200, 'Linewidth',2)
xlabel('x')
ylabel('Temperature')
...
grid on
legend({'50x50','100x100', '200x200'},'Location','northeast')
title('Centre-line Temperature using SOR for all cases')
...
hold off
legend({'50x50','100x100', '200x200'},'Location','northeast')
title('Log of Residual vs. Iterations using SOR for all cases')
...
grid on
hold off
title('Log of Residual vs. CPU Time using SOR for all cases')
